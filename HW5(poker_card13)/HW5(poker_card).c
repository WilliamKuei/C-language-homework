...
int main() {
    ...
    memcpy(PK_colar, PK, sizeof(PKCARD) * N);
    memcpy(PK_num, PK, sizeof(PKCARD) * N);

    printf("Check card sort in colar:\n");
    sort_cards_colar(PK_colar,N);
    print_cards(PK_colar, N);

    sort_cards_number(PK_num,N);
    printf("Check card sort in number:\n");
    print_cards(PK_num, N);

    bool straight=false;
    for(int i=0;i<N-4;i++){
        if(check_Straight(PK_num,i)){
            straight=true;
        }
    }
    if(!straight){
    ...
    for(int i=0;i<N-4;i++){
        if(check_Straight(PK_colar,i)&&PK_colar[i].hand_card_Type==PK_colar[i+4].hand_card_Type){
            Straight_Flush=true;
        }
    }
    if(Straight_Flush){
